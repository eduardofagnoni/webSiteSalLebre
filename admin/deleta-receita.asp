...
oConexaoDelete.AbreTabela("select foto from "&oConexaoDelete.prefixoTabela&"receitas where ativo='s' and id="&id)
nomeDoArquivo = oConexaoDelete.rs("foto")

    dim fs
    Set fs=Server.CreateObject("Scripting.FileSystemObject")    
    MyFile = Server.mappath(upGalerias&"__media\receita\"&nomeDoArquivo)
...
'apagando registro
oConexaoDelete.UpdateItem("DELETE from "&oConexaoDelete.prefixoTabela&"receitas WHERE id="&id)

response.redirect("lista-receita.asp?msg_sys=45")
