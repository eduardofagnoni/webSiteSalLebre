...
oListaReceita.AbreTabela("select id,idCategoria,nome,foto,data from "&oListaReceita.prefixoTabela&"receitas where ativo='s' AND regTerminado='s' order by nome asc")
                ...
                <div class="box">
                    <div class="box-header">
                        ...
                        <div style="float:right;">
                            <a href="add-receita.asp" class="btnExtBt btAzul" style="display:block">                                    
                                ...
                                <tr>
                                    <td>
                                        ...
                                        if oListaReceita.rs("foto")<>"" then
                                            response.write("<img src='"&oListaReceita.enderecoReceita&oListaReceita.rs("foto")&"' alt='' style='width:100%;'>")
                                        else
                                            response.write("<img src='images/user.jpg' alt='' style='width:100%;'>")
                                        end if
                                        %>
                                    </td>
                                    <td><%=oListaReceita.rs("nome")%></td>
                                    <td><%=oListaReceita.rs("data")%></td>
                                    <td>Tipo da Receita</td>
                                    <td><a href="add-receita.asp?regIni=s&id=<%=oListaReceita.rs("id")%>" class="btn btn-block btn-primary">Editar</a></td>
                                    <td><a href="deleta-receita.asp?id=<%=oListaReceita.rs("id")%>" class="btn btn-block btn-warning">Excluir</a></td>
                                </tr>
                                <%
                                oListaReceita.rs.MoveNext()
                                wend
                                oListaReceita.rs.Close()
                                set oListaReceita.rs = nothing
                                %>                       
                            </tbody>                        
                        </table>
                    </div>
