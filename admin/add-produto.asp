...
if regIniciado="s" then
    ...
    oEditRegistro.AbreTabela("SELECT id,idTipo,idUso,nome,resumo,embalagens,foto FROM "&oEditRegistro.prefixoTabela&"produto WHERE ativo='s' AND id="&id)

    if oEditRegistro.rs("idTipo")<>"" then
        
        oConexaoListas.AbreTabela("select id,nome from "&oConexaoListas.prefixoTabela&"tipo_de_sal where ativo='s' AND id="&oEditRegistro.rs("idTipo"))
        
        if oConexaoListas.rs.eof=false then
            varIdTipoSalRecuperado = oConexaoListas.rs("id")
            varNomeTipoSalRecuperado = oConexaoListas.rs("nome")
        else
            varIdTipoSalRecuperado = ""
            varNomeTipoSalRecuperado = "Selecione o Tipo de Sal"
        end if 
        
    end if

    if oEditRegistro.rs("idUso")<>"" then

        oConexaoListas.AbreTabela("select id,nome from "&oConexaoListas.prefixoTabela&"uso_do_sal where ativo='s' AND id="&oEditRegistro.rs("idUso"))

        if oConexaoListas.rs.eof=false then
            varIdUsoSalRecuperado = oConexaoListas.rs("id")
            varNomeUsoSalRecuperado = oConexaoListas.rs("nome")
        else
            varIdUsoSalRecuperado = ""
            varNomeUsoSalRecuperado = "Selecione o Uso do Sal"
        end if
    ...
    oAddRegistro.AddItem("INSERT INTO "&oAddRegistro.prefixoTabela&"produto (data, userIdSession, ultimaAlteracao) values ('"&dataInclusao&"','"&idUser&"','"&ultima_alteracao&"')")
    oEditRegistro.AbreTabela("SELECT id FROM "&oEditRegistro.prefixoTabela&"produto WHERE ativo='s' AND userIdSession="&idUser&" AND registroIncluido='n' ORDER BY id DESC")
    response.redirect("add-produto.asp?regIni=s&id="&oEditRegistro.rs("id"))
end if
    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper">
            ...
            <section class="content-header">
                <h1>Adicionar Produto <small></small></h1>
                <ol class="breadcrumb">
                    <li><a href="admin.asp"><i class="fa fa-home"></i> Home</a></li>
                    <li><a href="lista-produto.asp"><i class="fa fa-list"></i> Lista de Produto</a></li>
            ...
            <section class="content container-fluid">
                        ...
                        <div class="box-body">
                            ...
                            <figure style="margin-bottom:20px;text-align:center">
                                ...
                                <figcaption style="margin-top:10px">
                                    ...
                                    <br>
                                    <small style="display:block; margin: 10px 0 0 0;"><strong>A imagem do produto deve ser um .png com fundo transparente, e suas dimensões devem ser 436 x 609 pixels.</strong></small>
                                </figcaption>                            
                            </figure>
                ...
                <div class="col-xs-12 col-md-9">
                    <div class="box">
                        ...
                        <div class="box-body">
                            <form class="form-horizontal form-label-left input_mask" method="post" action="gravar-produto.asp">  

                                <div class="row">
                                    <div class="col-xs-12 col-md-4">
                                        <div class="form-group">
                                            <label class="sr-only" for="txtTipoDeSal">Tipo de Sal</label>
                                            <select class="form-control" id="txtTipoDeSal" name="txtTipoDeSal" required>
                                                <%
                                                if varIdTipoSalRecuperado="" then
                                                    oConexaoListas.AbreTabela("select id,nome from "&oConexaoListas.prefixoTabela&"tipo_de_sal where ativo='s' AND regTerminado='s' order by nome asc")
                                                %>
                                                <option value="" selected="selected">Selecione o Tipo de Sal</option>
                                                <%
                                                else
                                                    oConexaoListas.AbreTabela("select id,nome from "&oConexaoListas.prefixoTabela&"tipo_de_sal where ativo='s' AND regTerminado='s' AND id<>"&varIdTipoSalRecuperado&" order by nome asc")
                                                %>
                                                <option value="<%=varIdTipoSalRecuperado%>" selected="selected"><%=varNomeTipoSalRecuperado%></option>
                                                <%
                                                end if                                                
                                    ...
                                    <div class="col-xs-12 col-md-4">
                                        <div class="form-group">
                                            <label class="sr-only" for="txtUsoDoSal">Uso do Sal</label>
                                            <select class="form-control" id="txtUsoDoSal" name="txtUsoDoSal" required>
                                                <%
                                                if varIdUsoSalRecuperado="" then
                                                    oConexaoListas.AbreTabela("select id,nome from "&oConexaoListas.prefixoTabela&"uso_do_sal where ativo='s' AND regTerminado='s' order by nome asc")
                                                %>
                                                <option value="" selected="selected">Selecione o Uso do Sal</option>
                                                <%
                                                else
                                                    oConexaoListas.AbreTabela("select id,nome from "&oConexaoListas.prefixoTabela&"uso_do_sal where ativo='s' AND regTerminado='s' AND id<>"&varIdUsoSalRecuperado&" order by nome asc")
                                                %>
                                                <option value="<%=varIdUsoSalRecuperado%>" selected="selected"><%=varNomeUsoSalRecuperado%></option>
                                                <%
                                                end if                                                
                                                while not oConexaoListas.rs.eof
                                                response.write("<option value='"&oConexaoListas.rs("id")&"'>"&oConexaoListas.rs("nome")&"</option>")
                                ...
                                <div class="row">
                                    <div class="col-xs-12">                                                            
                                        ...
                                        <div class="form-group">	      
                                            <label for="txtEmbalagem">Embalagens</label>
                                            <textarea name="txtEmbalagem" id="txtEmbalagem" class="form-control" placeholder="Embalagens"><%=oEditRegistro.rs("embalagens")%></textarea>
                                            <script type="text/javascript">
                                                CKEDITOR.replace( "txtEmbalagem", { 
                                ...
                                <div class="form-group" style="float:right">
                                    <button type="button" class="btnExtBt btTransparent" onclick="location.href='lista-produto.asp?msg_sys=44'">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                        ...
                        <div class="profile_img">
                            <div id="crop-avatar">                                
                                <form action="gravar-foto-produto.asp?id=<%=oEditRegistro.rs("id")%>" class="dropzone" id="my-dropzone-fotoTopoProjeto" method="post" enctype="multipart/form-data">
                                    <div class="fallback">
                ...
                <div class="modal-footer">            
                    <button type="button" class="btnExtBt btAzul" data-dismiss="modal" onclick="location.href='add-produto.asp?regIni=s&id=<%=oEditRegistro.rs("id")%>'">
                        <div class="bg2"><img src="images/btGeralBr.png" alt=""></div>
                        Fechar e Atualizar
                    </button>        
                </div>
...
<script src="main-produto.js"></script>	
