...
Dim oListaProjetos
Set oListaProjetos = New Conexao
oListaProjetos.AbreConexao()
oListaProjetos.AbreTabela("select id,nome,foto,data from "&oListaProjetos.prefixoTabela&"projeto where ativo='s' AND regTerminado='s' order by nome asc")
                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <%
                                while not oListaProjetos.rs.eof                                     
                                %>
                                <tr>
                                    <td>
                                        <%
                                        if oListaProjetos.rs("foto")<>"" then
                                            response.write("<img src='"&oListaProjetos.enderecoProjetos&oListaProjetos.rs("foto")&"' alt='' style='width:100%;'>")
                                        else
                                            response.write("<img src='images/user.jpg' alt='' style='width:100%;'>")
                                        end if
                                        %>
                                    </td>
                                    <td><%=oListaProjetos.rs("nome")%></td>
                                    <td><%=oListaProjetos.rs("data")%></td>
                                    <td><a href="add-projetos.asp?regIni=s&id=<%=oListaProjetos.rs("id")%>" class="btn btn-block btn-primary">Editar</a></td>
                                    <td><a href="deleta-projetos.asp?id=<%=oListaProjetos.rs("id")%>" class="btn btn-block btn-warning">Excluir</a></td>
                                </tr>
                                <%
                                oListaProjetos.rs.MoveNext()
                                wend
                                oListaProjetos.rs.Close()
                                set oListaProjetos.rs = nothing
                                %>                       
                            </tbody>                        
                        </table>
                    </div>
